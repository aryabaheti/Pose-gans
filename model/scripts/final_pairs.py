...
heatmaps_dir = r"C:\Users\Arya Baheti\Desktop\test\data\keypoints\heatmaps"

# ✅ Save outputs in your own user directory (not hp)
base_output_dir = r"C:\Users\Arya Baheti\Desktop\test\data\splits"
out_train_pairs = os.path.join(base_output_dir, "train_pairs.csv")
out_val_pairs   = os.path.join(base_output_dir, "val_pairs.csv")

val_split_ratio = 0.1  # fraction of identities for validation
# --------------------

def read_identities_from_filenames(heatmaps_path):
    """
    ...
    if not os.path.exists(heatmaps_path):
        print(f"Error: Directory not found at {heatmaps_path}")
        return pid2imgs

    files = sorted(os.listdir(heatmaps_path))
    for file in files:
        if not file.lower().endswith(".png"):  # Ensure case-insensitive check for .png
            continue
...
def main():
    pid2imgs = read_identities_from_filenames(heatmaps_dir)
    ...
    print("✅ Training and validation pair CSVs generated successfully!")
